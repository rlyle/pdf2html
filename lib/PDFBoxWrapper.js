...
class PDFBoxWrapper {
    static async generateImage(filepath, options) {
        ...
        return FileManager.withTempFile(filepath, DEFAULT_OPTIONS.DIRECTORY.PDF, async (tempFilePath, tempUri) => {
            ...
            const finalImagePath = path.join(DEFAULT_OPTIONS.DIRECTORY.IMAGE, tempUri.filename().replace(tempUri.suffix(), opts.imageType));
    ...
    static async executePDFBox(filepath, options) {
        const args = ['-jar', path.join(DEFAULT_OPTIONS.DIRECTORY.VENDOR, DEFAULT_OPTIONS.VENDOR_PDF_BOX_JAR), 'PDFToImage', '-imageType', options.imageType, '-startPage', options.page.toString(), '-endPage', options.page.toString(), filepath];
        ...
        await CommandExecutor.execute(DEFAULT_OPTIONS.command.javaBin, args, {
    ...
    static async extractAllImages(filepath, options = {}) {
        const outputDirectory = options.outputDirectory || DEFAULT_OPTIONS.DIRECTORY.IMAGE;
        ...
        const args = [
            '-jar',
            path.join(DEFAULT_OPTIONS.DIRECTORY.VENDOR, DEFAULT_OPTIONS.VENDOR_PDF_BOX_JAR),
            'ExtractImages',
            '-prefix',
            prefix,
            filepath,
